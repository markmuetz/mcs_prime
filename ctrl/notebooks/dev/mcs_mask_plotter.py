...
class McsMaskPlotterData:
    def __init__(self, task, times):
        self.task = task
        self.gen_region_masks = task.depends_on[-1]
        self.times = times
        assert (times.year == task.year).all()
        assert (times.month == task.month).all()
        ...
        self.pixel_on_e5_paths = [task.inputs[f'e5pixel_{t}']
                                  for t in times]
        self.tracks_path = self.task.inputs['tracks']

    def load(self):
        self.tracks = McsTracks.open(self.tracks_path, None)
        self.pixel_on_e5 = xr.open_mfdataset(self.pixel_on_e5_paths, concat_dim='time', combine='nested').load()

        mcm, msm, ccm, csm, em = self.gen_region_masks(self.logger, self.pixel_on_e5, self.tracks)
        self.mcm = mcm
        self.msm = msm
        self.ccm = ccm
        self.csm = csm
        ...
        print(self.e5times)
        path_fmt = (e5datadir / 'ecmwf-era5_oper_an_sfc_{year}{month:02d}{day:02d}{hour:02d}00.{var}.nc')
        e5paths = [fmtp(path_fmt, year=t.year, month=t.month, day=t.day, hour=t.hour, var=var)
                   for var in ['cape', 'tcwv']
                   for t in self.e5times]
        self.e5data = (xr.open_mfdataset(e5paths).sel(latitude=slice(60, -60)).interp(time=self.times)).load()
