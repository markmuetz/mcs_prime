...
def check_cloudnumbers(dspf):
    ...
    for track_index, track_id in enumerate(dspf.tracks):
        ...
        print(
            f'{track_index + 1}/{len(dspf.tracks)}',
            track.base_time.values[0],
            track_duration,
        )
...
def plot_tracks_at_time(
    ...
    for track_index, track_id in enumerate(dspf.tracks):
        ...
        if display_track:
            ...
            if colour_marker:
                ax.plot(
                    ...
                    track.meanlat[track_duration - 1],
                    ''.join(colour_marker),
                )
        ...
        ax.annotate(
            marker,
            (track.meanlon[time_index].values, track.meanlat[time_index].values),
            fontsize='x-small',
        )
        ...
        if display_trackresult:
            ...
            if colour_marker:
                ax.plot(
                    track.meanlon[time_index],
                    track.meanlat[time_index],
                    ''.join(colour_marker),
                )
        if legend_elements:
